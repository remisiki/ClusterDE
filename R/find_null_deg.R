...
#' @param normalize Logical; if \code{TRUE} (default), apply Seurat's NormalizeData to each dataset. If \code{FALSE}, sets assay data directly. Set to false if null data is generated using PCA approximation.
...
#'
#' @export
findNullDeg <- function(synthetic_null, normalize = T, seed = 123) {
  if (!is.list(synthetic_null)) {
    synthetic_null <- list(synthetic_null)
  }
  null_pval_list <- lapply(synthetic_null, function(count_mat) {
    set.seed(seed)
